...
def ProgressBar(Total, Progress, BarLength=20, ProgressIcon="#", BarIcon="-"):
    try:
        # progress bars minimum length is 1
        if BarLength <1:
            BarLength = 20
        # use Status variable for moving to the next line after progress completion
        Status = ""
        # calculating progress as a percentage between 0 and 1
        ...
        # format and return the progress bar str
...
# function to display text character by character and apply tracking if needed
...
# function to compute the optimal font size based on the image dimensions and font style
...
def getColors(hexaCol):
    ...
    # calculating luminance using standard formula
    lumi=rY * 0.2126 + gY * 0.7152 + bY * 0.0722

    if lumi > 160:
        return ("#000000","#090909")
    else:
        return ("#ffffff","#fafafa")

""" GLOBAL PARAMETERS SECTION """

# full color index in Logic Pro X: not currently used, reserved for later
# all columns and lines match the position of each color in the Logic Pro color grid + 5 shades of grey
# remove colors from the list if you want to create fewer variations
logicColors=[ 
...
# reference font; font must be present in C:\Windows\Fonts or the Font directory on Mac
# condensed bold fonts yield better results: Avenir Next, Futura Medium Condensed, Arial Narrow Bold, DIN Condensed Bold, Hattenschweiler, etc
...
# overlay picture (inner shadow/emboss); this image must match the exact size of the output (e.g., emboss512.png, emboss64.png)
# if the overlay image size doesn't match the icon size, the script will produce an error
overlay=Image.open('emboss'+str(imgSize)+'.png').convert("RGBA")

# if set to 0 only the color associated with the icon definition list is generated
# if set to 1 the icon will be generated for each Logic color; this takes more resources and disk space
# in this case text color is adapted based on each color's luminance (black vs white text)
generateAllColors=0

""" END OF PARAMETERS"""

if not os.path.exists("icons"+str(imgSize)): # creates the icons dir if it doesn't exist
   os.makedirs("icons"+str(imgSize))

for f in glob.glob("icons"+str(imgSize)+"/*.png"): # delete existing images
    os.remove(f)
...
for idxi,i in enumerate(data['tracks'],start=1): # browsing each track (line)
    text=i["main"] # main text for icon (1st line, best between 2-4 characters)
    style=i["style"] # style: normal (0), or group (has outline) for ensembles, or folder for Logic folders (lower height)
    ...
    for text2 in i["sub"]: # browsing variations within tracks, filling 2nd line
        ...
        if generateAllColors==1: # retrieving icon background color
            ...
            if len(colorList)==0:
                colorList.append(i["color"]) # in standard mode (icons only generated in their associated color), a 1 item list is created; if empty a color is added, when not empty the color is updated
            else:
                colorList[0]=i["color"]
        ...
        for idxc,bgcolor in enumerate(colorList,start=1):
            ...
            (size1,yoffset1) = optimize_fontsize(dst_img,text,int(imgSize*0.375),int(imgSize*0.5625),style,fontRef) # calculating ideal font size
            ...
            draw_text_psd_style(draw, xy=(int(imgSize*0.05), yoffset1*imgheight/imgSize), font=font1,text=text,
                            tracking=0, leading=10, fill=color1) # drawing first line

            if style!="folder": # 2nd line for folders
                draw_text_psd_style(draw, xy=(text2offset, int(imgSize*0.53125)+yoffset2), font=font2,text=text2,
                            tracking=0, leading=10, fill=color2) # draw 2nd line

            outlineWidth=int(imgSize*0.03125) # calculating outline width

            if style!=0: # drawing outline for group & folder tracks
